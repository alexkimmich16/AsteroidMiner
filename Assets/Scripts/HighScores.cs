...
{
    public string privateCode = "Private Key";  //Key to Upload New Info
    public string publicCode = "Public Key";   //Key to download
    ...
    {
        int Adjusted = (-1 * score) + 1000000;
        WWW www = new WWW(webURL + privateCode + "/add/" + WWW.EscapeURL(userame) + "/" + Adjusted + "/" + Adjusted);
        yield return www;
    ...
    IEnumerator DatabaseDownload()
    {
        ...
        //WWW www = new WWW(webURL + publicCode + "/pipe/0/10-seconds-asc"); //Gets top 10
        yield return www;
    ...
    void OrganizeInfo(string rawData) //Divides Scoreboard info by new lines
    {
        ...
        for (int i = 0; i < entries.Length; i ++) //For each entry in the string array
        {
            ...
            int UnAdjusted = int.Parse(entryInfo[1]);
            //turn high to low
            int score = 1000000 - UnAdjusted;
